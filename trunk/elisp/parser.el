...
;; A separate scanner must be built, and it generates a source file.
;; This is typical of bottom up approaches which decompose a parser
;; down to optimized finite automata.

;; * Elisp Parser

;; Elisp parser does not generate a parser, but provides tools to do
;; so. Productions and terminals are specified as macros which are
;; then expanded by normal lisp evaluation. I think of this technique
;; as an "inside out" macro, since the specification of the parser is
;; inside the macros, while the generation of the parser is outside
;; the macro.

;; ->Comparison

;; The tools above disconnect definition from generation which is not
;; very lispy. Any bottom up parser will also require a separate
;; scanner which is more complexity and also less intuitive as the
;; definition of the parser is now split and connected by API.

;; With my macro approach evaluating the grammar produces a parser
;; directly and no scanner is required allowing the grammar to be a
;; complete definition of the parser.
...
;; 2. Parse Data is orthogonal to the parser and not hard-wired
;;    in. Tokens can perform user defined functions or construct
;;    data from the bounds of a token match in the input.

;;    The AST data structure is a tree generated by the parser.
;;    non-terminal productions add nodes, tokens add leaves.

;;    The flexibility of tokens are important for parsing in Emacs
;;    which possesses a truly astounding tool-set.
...
;; The concept started with the idea to build a pseudo-parser as
;; nested cond forms. The matching would go in the predicate part and
;; the action in the body.

;; This early experiment performed a simple recursion across the
;; sexp's given to the macro. This experiment made the design
;; of the parser compiler obvious. Unfortunately the concise
;; expression of the original experiment: <60 lines, was sacrificed
;; for the necessary complexity of a full-featured parser.

;; The next major milestone was converting the cond clause form of
;; matching to lambda's, and using a dynamically scoped objarray
;; symbol table for the productions. This allowed previously defined
;; terminals and non-terminals to be used in matching rules.

;; Operators were next. I implemented and/or as primitive building
;; blocks. This gave it PEG characteristics as ordered choice is
;; a roundabout way of saying or.

;; to implement operators like conditional I began implementing
;; predicate functions. Luckily a friend mentioned PEG's by pure
;; chance, and took a look at the work on PEG grammars. The original
;; paper on PEG's was very helpful in defining the essential set of
;; predicates required to approximate the power of a CFG grammar
;; without the side-effects of CFG complexity.
